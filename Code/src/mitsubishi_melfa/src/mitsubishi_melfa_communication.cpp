...
double duration_send, duration_rcv;
...
int main(int argc, char **argv) {
    ...
    double old_sec_send = 0;
    double old_sec_rcv = 0;
    ...
    while(loop && ros::ok() && bSocketStatus)
    {
            ...
            // Duration between sends
            double secs_send =ros::Time::now().toSec();
            duration_send = secs_send - old_sec_send;
            if (duration_send > 0.0071)
            {
	            std::cout << "Duration between sends " << duration_send 
	            	<< " exceeded 7.1 ms" << std::endl;
            }
            old_sec_send = secs_send;
            ...
            while(retry)
            {
                ...
                if(bRobotCompletedCommand)
                {
		            ...
		            // Duration between receives
                    double secs_rcv =ros::Time::now().toSec();
		            duration_rcv = secs_rcv - old_sec_rcv;
		            if (duration_rcv > 0.0071)
		            {
			            std::cout << "Duration between receives " << duration_rcv
			            << " exceeded 7.1 ms" << std::endl;
		            }
		            old_sec_rcv = secs_rcv;

		            std::cout << "Time between send and receive "
		            	<< secs_rcv - secs_send << std::endl;
